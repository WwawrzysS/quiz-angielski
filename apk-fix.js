...
(function () {
    ...
    async function nativeSpeak(text) {
        ...
        if (NativeTextToSpeech) {
            try {
                if (typeof NativeTextToSpeech.stop === "function") {
        ...
        try {
            if (typeof SpeechPlugin.checkPermissions === "function") {
                ...
                if (
                    checked &&
                    (
                        checked.microphone === "granted" ||
                        checked.speechRecognition === "granted"
                    )
                ) {
                    return true;
                }
    ...
    window.startListening = async function (btn) {
        ...
        try {
            if (typeof SpeechPlugin.removeAllListeners === "function") {
            ...
            if (typeof SpeechPlugin.addListener === "function") {
                await SpeechPlugin.addListener("partialResults", function (event) {
                    if (event && Array.isArray(event.matches) && event.matches.length > 0) {
                        ...
                        if (finalText) {
                            preview.innerText = 'Słyszę: "' + finalText + '"';
                        }
                    }
            ...
            setTimeout(async function () {
                try {
                    if (typeof SpeechPlugin.getLastPartialResult === "function") {
                        ...
                        if (last && typeof last.partialResult === "string" && last.partialResult.trim()) {
                            finalText = last.partialResult.trim();
                        }

                        if (last && Array.isArray(last.matches) && last.matches.length > 0) {
                            finalText = String(last.matches[0] || "").trim();
                        }
                    }
                } catch (error) {}
                ...
                if (typeof verifySpeech === "function") {
                    verifySpeech(finalText, btn);
                } else {
    ...
    window.stopSpeechEngine = async function () {
        if (typeof clearSpeechHints === "function") {
            clearSpeechHints();
        }
    ...
    try {
        recognition = { nativeCapacitor: true };
    } catch (error) {}
})();
